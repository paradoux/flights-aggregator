...
//Store all mock data and expected results in different file
const {
  airMoonMockResult,
  airJazzMockResult,
  airBeamMockResult,
...
describe("Routes tests", () => {
  describe("GET /api/flights", () => {
    test("Should return sorted results", async () => {
      ...
      // Check response from server same as expected response
    ...
    test("Should return 'Can't get flights from the following provider API: air-moon'", async () => {
      ...
      expect(res.body).toEqual({
        err: {
          message:
            "Can't get flights from the following provider API: air-moon",
          status: 500
        }
      })
    })
